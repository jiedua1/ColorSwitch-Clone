...
public class RotatingPlusOther extends Actor{
	...
	public RotatingPlusOther(Rectangle viewPort, double position, double xPos, double angle, double angularSpeed, double length) {
	...
	public void initPieces() {
		...
		double[] xPoints = {xPos, xPos+length, xPos+length, xPos};
		double[] yPoints = {position, position, position+length/10, position+length/10};
		for(int i = 0; i<4; i++) {
			RotatingSegment piece = new RotatingSegment(viewPort, position, 180-90*i, xPoints, yPoints);
			piece.setPivot(xPos+length/50, position+length/20);
