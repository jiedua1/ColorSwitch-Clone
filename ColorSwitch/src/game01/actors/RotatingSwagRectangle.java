...
public class RotatingSwagRectangle extends Actor{
	...
	public void initPieces(double theta) {
		...
		for(int i = 0; i<4; i++) {
			...
			piece.setPivot(xPos, position);
			piece.setRotate();
			if(i == 0) piece.translate(0,0);
			if(i == 1) piece.translate(0, 0);
			if(i == 2) piece.translate(length, length);
			if(i == 3) piece.translate(length, length);
			piece.setPivot(xPos, position);
	...
	public void render(Graphics2D g) {
		...
		if(increasing) length += 1; else length -=1;
		angle++;
		initPieces(angle);
		for(int i = 0; i<pieces.size(); i++) {
