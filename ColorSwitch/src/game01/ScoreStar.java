...
public class ScoreStar extends Actor{ 
	...
	int[] xList;
	int[] yList;
	public ScoreStar(Rectangle viewPort, double position, double xPos, double s, int edges) {
		...
		for(int i = 0; i<edges; i++) {
			...
			if(i%2==0) {
				xList[i]= (int) (xPos + size*Math.cos(Math.toRadians(angle)));
				yList[i] = (int) (position + size*Math.sin(Math.toRadians(angle)));
			} else {
				xList[i]= (int) (xPos + 0.5 * size*Math.cos(Math.toRadians(angle)));
